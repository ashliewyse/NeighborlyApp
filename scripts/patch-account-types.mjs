...
replaceOnce(
  ...
  `          ) : step === 1 ? (\n            <>\n              <div>\n                <label className={labelClass}>Account Type</label>\n                <p className="text-xs text-muted-foreground mb-2">Choose how you want to use Neighborly.</p>\n                <div className="grid grid-cols-2 gap-3">\n                  <button\n                    type="button"\n                    onClick={() => setAccountType("personal")}\n                    className={\`rounded-xl border p-4 text-left transition-all \${accountType === "personal" ? "border-blue-600 bg-blue-50 ring-1 ring-blue-600" : "border-border hover:border-blue-300"}\`}\n                  >\n                    <div className="text-2xl mb-1">👤</div>\n                    <div className="font-semibold text-sm">Personal Account</div>\n                    <div className="text-xs text-muted-foreground mt-1">Connect, post, help neighbors, and build your profile.</div>\n                  </button>\n                  <button\n                    type="button"\n                    onClick={() => setAccountType("business")}\n                    className={\`rounded-xl border p-4 text-left transition-all \${accountType === "business" ? "border-blue-600 bg-blue-50 ring-1 ring-blue-600" : "border-border hover:border-blue-300"}\`}\n                  >\n                    <div className="text-2xl mb-1">🏪</div>\n                    <div className="font-semibold text-sm">Local Business</div>\n                    <div className="text-xs text-muted-foreground mt-1">Create a business profile and connect with local customers.</div>\n                  </button>\n                </div>\n              </div>\n              <div>\n                <label className={labelClass}>{accountType === "business" ? "Owner / Contact Name" : "Full Name"}</label>`,
...
replaceOnce(
  ...
  `          ) : (\n            <>\n              {accountType === "business" && (\n                <div className="rounded-xl border border-border bg-muted/30 p-4 flex flex-col gap-3">\n                  <div>\n                    <label className={labelClass}>Business Name</label>\n                    <input className={inputClass} value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="Beachside Cleaners" />\n                  </div>\n                  <div>\n                    <label className={labelClass}>Business Category</label>\n                    <input className={inputClass} value={businessCategory} onChange={(e) => setBusinessCategory(e.target.value)} placeholder="Home Services, Cleaning, Lawn Care…" />\n                  </div>\n                  <div className="grid sm:grid-cols-2 gap-3">\n                    <div>\n                      <label className={labelClass}>Phone <span className="normal-case font-normal">(optional)</span></label>\n                      <input className={inputClass} value={businessPhone} onChange={(e) => setBusinessPhone(e.target.value)} placeholder="(219) 555-0123" />\n                    </div>\n                    <div>\n                      <label className={labelClass}>Website <span className="normal-case font-normal">(optional)</span></label>\n                      <input className={inputClass} value={businessWebsite} onChange={(e) => setBusinessWebsite(e.target.value)} placeholder="yourbusiness.com" />\n                    </div>\n                  </div>\n                  <div>\n                    <label className={labelClass}>About Your Business <span className="normal-case font-normal">(optional)</span></label>\n                    <textarea className={\`\${inputClass} min-h-20 resize-none\`} value={businessDescription} onChange={(e) => setBusinessDescription(e.target.value)} placeholder="Tell neighbors what your business does…" maxLength={500} />\n                  </div>\n                </div>\n              )}\n              <div className="grid sm:grid-cols-2 gap-3">`,
