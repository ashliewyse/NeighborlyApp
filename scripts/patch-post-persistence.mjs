...
const renderedAvatarWithOwnOnly = '<Avatar name={post.author} size="md" src={post.author === (currentAccountType === "business" ? currentBusiness?.name : currentProfile?.name) ? myAvatarUrl : null} />';
const renderedAvatar = '<Avatar name={post.author} size="md" src={post.authorAvatar || (post.author === (currentAccountType === "business" ? currentBusiness?.name : currentProfile?.name) ? myAvatarUrl : null)} />';
if (s.includes(renderedAvatarWithOwnOnly)) s = s.replaceAll(renderedAvatarWithOwnOnly, renderedAvatar);
s = s.replaceAll('<Avatar name={post.author} size="md" />', renderedAvatar);
s = s.replaceAll('<Avatar name={post.author} size="sm" />', '<Avatar name={post.author} size="sm" src={post.authorAvatar || null} />');

fs.writeFileSync(file, s);
console.log('Patched feed posts to persist and reload with each author avatar from Supabase.');
